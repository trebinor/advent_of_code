...
pub fn original_03a(input: &str) -> u32 {
    ...
    mut_grid_03a(&mut grid1, v1);
    mut_grid_03a(&mut grid2, v2);
...
pub fn original_03b(input: &str) -> u32 {
    let v: Vec<&str> = input.split('\n').collect();
    let mut v1 = v[0].trim().split(',').collect();
    let mut v2 = v[1].trim().split(',').collect();
    let mut grid1 = vec![vec![false; GRID_X]; GRID_Y];
    let mut grid2 = vec![vec![false; GRID_X]; GRID_Y];
    mut_grid_03b(&mut grid1, &mut v1);
    mut_grid_03b(&mut grid2, &mut v2);
...
fn mut_grid_03a(grid: &mut Vec<Vec<bool>>, wires: Vec<&str>) {
...
fn mut_grid_03b(grid: &mut Vec<Vec<bool>>, wires: &mut Vec<&str>) {
    let mut grid_point: GridPoint = GridPoint {
        x: GRID_X / 2,
        y: GRID_Y / 2,
    };
